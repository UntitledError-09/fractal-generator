 ...
 * 
 * This class provides a high-level interface for working with shaders in Vulkan.
 * It handles the compilation process from GLSL to SPIR-V and manages shader
 * module lifecycle.
 * 
 * Key Features:
...
public:
     ...
     */
    std::shared_ptr<ShaderInfo> compileShader(
     ...
     * @return Pointer to ShaderInfo containing compiled shader data
     * 
     * @throws std::runtime_error If file reading, compilation, or module creation fails
     */
    std::shared_ptr<ShaderInfo> loadShaderFromFile(
        const std::string& name,
     ...
     */
    std::shared_ptr<ShaderInfo> createShaderModule(
        const std::string& name,
    ...
    // TODO(Phase 3): Add shader hot-reloading support
...
private:
    ...
    std::unordered_map<std::string, std::shared_ptr<ShaderInfo>> m_shaders; ///< Cached shaders
};
