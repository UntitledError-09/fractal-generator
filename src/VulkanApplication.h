 ...
 * the various subsystems and provides a clean interface for the main()
 * function.
 *
 * Design Principles:
...
private:
   ...
   *
   * Handles GLFW window creation, event processing, and Vulkan surface
   * creation. Encapsulates all windowing system interactions.
   */
  ...
  /**
   * @brief GUI management for interactive parameter controls
   *
   * Manages Dear ImGui integration for real-time fractal parameter
   * modification and user interface rendering.
   */
  ...
  struct {
    ...
    uint32_t fractalType = 0; ///< Fractal type (0=Mandelbrot, 1=Julia, etc.)
  } m_fractalParams;

  /**
   * @brief GUI fractal parameters for user interface binding
   *
   * This structure bridges the UI controls with the actual computation
   * parameters. When GUI parameters change, they are copied to m_fractalParams
   * for computation.
   */
  struct {
    float centerX = -0.5f;
  ...
  //  Fractal computation state implemented
  // Parameters, zoom, center position, iteration count all working via GUI

  //  Rendering state implemented
  // Pipelines, descriptor sets, command buffers functional and operational
