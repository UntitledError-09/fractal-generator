 ...
 * management. It provides a clean abstraction over Vulkan's verbose setup process.
 * 
 * Phase 1 Focus:
...
struct QueueFamilyIndices {
    ...
    std::optional<uint32_t> computeFamily;   ///< Compute operations (fractal calculation)
...
struct PhysicalDeviceInfo {
    ...
    VkPhysicalDeviceProperties properties;      ///< Device properties (name, type, etc.)
