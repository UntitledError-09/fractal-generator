...
ComputePipeline::ComputePipeline(
    ...
    , m_fractalPipeline(VK_NULL_HANDLE)
    , m_fractalPipelineLayout(VK_NULL_HANDLE)
    , m_fractalDescriptorSetLayout(VK_NULL_HANDLE)
    , m_descriptorPool(VK_NULL_HANDLE)
    , m_fractalDescriptorSet(VK_NULL_HANDLE)
    , m_fractalImageWidth(0)
    , m_fractalImageHeight(0)
    , m_fractalPipelineReady(false)
{
    std::cout << "[ComputePipeline] Initialized compute pipeline system" << std::endl;
...
ComputePipeline::~ComputePipeline() {
    std::cout << "[ComputePipeline] Cleaning up compute pipeline resources..." << std::endl;
    ...
    if (m_fractalDescriptorSetLayout != VK_NULL_HANDLE) {
        vkDestroyDescriptorSetLayout(m_device, m_fractalDescriptorSetLayout, nullptr);
    }
...
bool ComputePipeline::createPipeline(const std::string& pipelineName, const std::string& shaderName) {
    ...
    if (!shader) {
        std::cerr << "[ComputePipeline] Shader not found: " << shaderName << std::endl;
        return false;
    }
    
    if (shader->type != ShaderType::COMPUTE) {
        std::cerr << "[ComputePipeline] Shader is not a compute shader: " << shaderName << std::endl;
        return false;
    }
    
    // TODO: Implement generic pipeline creation
    // For now, only fractal pipeline is implemented
    std::cerr << "[ComputePipeline] Generic pipeline creation not yet implemented" << std::endl;
    return false;
}

bool ComputePipeline::createFractalPipeline(uint32_t imageWidth, uint32_t imageHeight) {
    ...
    try {
        ...
        VkResult result = vkCreatePipelineLayout(m_device, &pipelineLayoutInfo, nullptr, &m_fractalPipelineLayout);
        if (result != VK_SUCCESS) {
            throw std::runtime_error("Failed to create pipeline layout! Vulkan error: " + std::to_string(result));
        }
        ...
        result = vkCreateComputePipelines(m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_fractalPipeline);
        if (result != VK_SUCCESS) {
            throw std::runtime_error("Failed to create compute pipeline! Vulkan error: " + std::to_string(result));
        }
        ...
        m_fractalParameterBuffer = m_memoryManager->createBuffer(
            "fractal_parameters",
            paramBufferSize,
            BufferUsage::FRACTAL_PARAMS_BUFFER,
            MemoryLocation::CPU_TO_GPU,
            true  // Persistently mapped for frequent updates
        );
        
        // Create output buffer
        size_t outputBufferSize = imageWidth * imageHeight * sizeof(uint32_t);  // RGBA32 format
        m_fractalOutputBuffer = m_memoryManager->createBuffer(
            "fractal_output",
            outputBufferSize,
            BufferUsage::FRACTAL_OUTPUT_BUFFER,
        ...
        m_fractalDescriptorSet = allocateAndUpdateDescriptorSet(
            m_fractalDescriptorSetLayout,
            m_fractalParameterBuffer,
            m_fractalOutputBuffer
        );
        
        m_fractalPipelineReady = true;
        
        std::cout << "[ComputePipeline] Fractal compute pipeline created successfully" << std::endl;
        std::cout << "[ComputePipeline] Parameter buffer: " << (paramBufferSize / 1024.0f) << " KB" << std::endl;
        std::cout << "[ComputePipeline] Output buffer: " << (outputBufferSize / (1024.0f * 1024.0f)) << " MB" << std::endl;
        
        return true;
        
    } catch (const std::exception& e) {
        std::cerr << "[ComputePipeline] Failed to create fractal pipeline: " << e.what() << std::endl;
        m_fractalPipelineReady = false;
        return false;
    }
}
...
void ComputePipeline::updateFractalParameters(const FractalParameters& params) {
    if (!m_fractalPipelineReady || !m_fractalParameterBuffer) {
        std::cerr << "[ComputePipeline] Fractal pipeline not ready for parameter updates" << std::endl;
        return;
    }
    
    // Copy parameters to the mapped buffer
    if (m_fractalParameterBuffer->mappedData) {
        std::memcpy(m_fractalParameterBuffer->mappedData, &params, sizeof(FractalParameters));
    } else {
        std::cerr << "[ComputePipeline] Parameter buffer not mapped!" << std::endl;
    }
}

void ComputePipeline::dispatchFractalCompute(
    VkCommandBuffer commandBuffer,
    uint32_t workGroupSizeX,
    uint32_t workGroupSizeY
) {
    if (!m_fractalPipelineReady) {
        std::cerr << "[ComputePipeline] Fractal pipeline not ready for dispatch" << std::endl;
        return;
    }
    
    // Calculate dispatch info
    ComputeDispatchInfo dispatchInfo = calculateDispatchInfo(
        m_fractalImageWidth,
        m_fractalImageHeight,
        workGroupSizeX,
        workGroupSizeY
    );
    
    // Bind compute pipeline
    vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, m_fractalPipeline);
    
    // Bind descriptor set
    vkCmdBindDescriptorSets(
        commandBuffer,
        VK_PIPELINE_BIND_POINT_COMPUTE,
        m_fractalPipelineLayout,
        0,
        1,
        &m_fractalDescriptorSet,
        0,
        nullptr
    );
    
    // Dispatch compute work
    vkCmdDispatch(commandBuffer, dispatchInfo.groupCountX, dispatchInfo.groupCountY, dispatchInfo.groupCountZ);
    
    // std::cout << "[ComputePipeline] Dispatched fractal compute: " 
    //           << dispatchInfo.groupCountX << "x" << dispatchInfo.groupCountY 
    //           << " work groups (" << workGroupSizeX << "x" << workGroupSizeY << " local size)" << std::endl;
}
...
bool ComputePipeline::getFractalData(std::vector<uint32_t>& outputData) {
    ...
    try {
        // Note: This requires the output buffer to be host-visible for direct download
        // In a real implementation, we might need a staging buffer for GPU-only memory
        m_memoryManager->downloadBufferData(
            m_fractalOutputBuffer,
            outputData.data(),
        ...
        std::cerr << "[ComputePipeline] Failed to download fractal data: " << e.what() << std::endl;
        return false;
    }
}
...
void ComputePipeline::getFractalDimensions(uint32_t& width, uint32_t& height) const {
...
VkDescriptorSetLayout ComputePipeline::createFractalDescriptorSetLayout() {
    ...
    if (result != VK_SUCCESS) {
        throw std::runtime_error("Failed to create descriptor set layout! Vulkan error: " + std::to_string(result));
    }
...
VkDescriptorPool ComputePipeline::createDescriptorPool(uint32_t maxSets) {
    ...
    if (result != VK_SUCCESS) {
        throw std::runtime_error("Failed to create descriptor pool! Vulkan error: " + std::to_string(result));
    }
...
VkDescriptorSet ComputePipeline::allocateAndUpdateDescriptorSet(
    ...
    if (result != VK_SUCCESS) {
        throw std::runtime_error("Failed to allocate descriptor set! Vulkan error: " + std::to_string(result));
    }
