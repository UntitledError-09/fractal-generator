...
{
    std::cout << "GraphicsPipeline: Initializing graphics pipeline..." << std::endl;
}
 ...
 */
GraphicsPipeline::~GraphicsPipeline() {
    std::cout << "GraphicsPipeline: Cleaning up graphics pipeline..." << std::endl;
...
bool GraphicsPipeline::createFractalDisplayPipeline() {
    std::cout << "GraphicsPipeline: Creating fractal display pipeline..." << std::endl;
    
    try {
        // Create descriptor set layout first
        if (!createDescriptorSetLayout()) {
            std::cerr << "GraphicsPipeline: Failed to create descriptor set layout!" << std::endl;
            return false;
        }
        
        // Create descriptor pool
        if (!createDescriptorPool()) {
            std::cerr << "GraphicsPipeline: Failed to create descriptor pool!" << std::endl;
            return false;
        }
        
        // Create descriptor set
        if (!createDescriptorSet()) {
            std::cerr << "GraphicsPipeline: Failed to create descriptor set!" << std::endl;
            return false;
        }
        ...
        if (!createFramebuffers()) {
            std::cerr << "GraphicsPipeline: Failed to create framebuffers!" << std::endl;
            return false;
        }
        
        m_pipelineReady = true;
        std::cout << "GraphicsPipeline: Fractal display pipeline created successfully." << std::endl;
        return true;
        
    } catch (const std::exception& e) {
        std::cerr << "GraphicsPipeline: Exception during pipeline creation: " << e.what() << std::endl;
        return false;
    }
}
 ...
 */
bool GraphicsPipeline::isPipelineReady() const {
 ...
 */
void GraphicsPipeline::beginRenderPass(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
 ...
 */
void GraphicsPipeline::renderFractal(VkCommandBuffer commandBuffer, VkImageView fractalTexture) {
 ...
 */
bool GraphicsPipeline::updateFractalTexture(VkImageView textureImageView, VkSampler textureSampler) {
...
void GraphicsPipeline::recreateForSwapchain() {
    std::cout << "GraphicsPipeline: Recreating pipeline for swapchain changes..." << std::endl;
    ...
    if (!createFramebuffers()) {
        std::cerr << "GraphicsPipeline: Failed to recreate framebuffers!" << std::endl;
        m_pipelineReady = false;
    }
}
...
bool GraphicsPipeline::createDescriptorSetLayout() {
    ...
    if (result != VK_SUCCESS) {
        std::cerr << "GraphicsPipeline: Failed to create descriptor set layout! Error: " << result << std::endl;
        return false;
    }
...
bool GraphicsPipeline::createRenderPass() {
    ...
    if (result != VK_SUCCESS) {
        std::cerr << "GraphicsPipeline: Failed to create render pass! Error: " << result << std::endl;
        return false;
    }
...
bool GraphicsPipeline::createPipeline() {
    ...
    auto fragmentShaderInfo = m_shaderManager->loadShaderFromFile(
        "fractal_display_fragment", 
        "shaders/fractal_display.frag", 
        ShaderType::FRAGMENT
    );
    if (!fragmentShaderInfo) {
        std::cerr << "GraphicsPipeline: Failed to load fragment shader!" << std::endl;
        return false;
    }
    ...
    if (result != VK_SUCCESS) {
        std::cerr << "GraphicsPipeline: Failed to create pipeline layout! Error: " << result << std::endl;
        return false;
    }
    ...
    if (result != VK_SUCCESS) {
        std::cerr << "GraphicsPipeline: Failed to create graphics pipeline! Error: " << result << std::endl;
        return false;
    }
...
bool GraphicsPipeline::createFramebuffers() {
    ...
    for (size_t i = 0; i < swapchainImages.size(); i++) {
        ...
        if (result != VK_SUCCESS) {
            std::cerr << "GraphicsPipeline: Failed to create framebuffer " << i << "! Error: " << result << std::endl;
            return false;
        }
    }
...
bool GraphicsPipeline::createDescriptorPool() {
    ...
    if (result != VK_SUCCESS) {
        std::cerr << "GraphicsPipeline: Failed to create descriptor pool! Error: " << result << std::endl;
        return false;
    }
...
bool GraphicsPipeline::createDescriptorSet() {
    ...
    if (result != VK_SUCCESS) {
        std::cerr << "GraphicsPipeline: Failed to allocate descriptor set! Error: " << result << std::endl;
        return false;
    }
